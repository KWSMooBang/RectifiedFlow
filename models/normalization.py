...
def get_normalization(config, conditional=False):
    ...
    if conditional:
        if norm == 'InstanceNorm++':
            return functools.partial(ConditionalInstanceNorm2dPlus, num_classes=config.model.num_class)
        else:
...
class ConditionalBatchNorm2d(nn.Module):
    def __init__(self, num_features, num_classes, bias=True):
        ...
        if self.bias:
            ...
            self.embed.weight.data[:, :num_features].uniform_() # initialize scale at N(1, 0.2)
...
class ConditionalInstanceNorm2d(nn.Module):
    def __init__(self, num_features, num_classes, bias=True):
        ...
        self.num_features = num_features
        self.bias = bias
        self.instance_norm = nn.InstanceNorm2d(num_features, affine=False, track_running_state=False)
        if bias:
            self.embed = nn.Embedding(num_classes, num_features * 2)
            self.embed.weight.data[:, :num_features].uniform_()
            self.embed.weight.data[:, num_features].zeros_()
        else:
            self.embed = nn.Embedding(num_classes, num_features)
            self.embed.weight.data.uniform_()
...
class ConditionalVarianceNorm2d(nn.Module):
    ...
    def forward(self, x, y):
        vars = torch.vars(x, dim=(2, 3), keepdim=True)
