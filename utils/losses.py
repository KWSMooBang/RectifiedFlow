    ...
    def optimize_fn(optimizer, params, step, lr=config.optim.lr, 
        ...
        if warmup > 0:
            for param_group in optimizer.param_groups:
                param_group['lr'] = lr * np.minimum(step / warmup, 1.0)
        if grad_clip >= 0:
...
def get_loss_fn(sde, train, reduce_mean=True, eps=1e-3):
    ...
    Args:
        sde: An object that represents the forward SDE
    ...
    def loss_fn(batch):
        ...
        if sde.reflow_flag:
            if sde.reflow_t_schedule == 't0': # distill for t = 0 (k = 1)
                t = torch.zeros(batch.shape[0], device=batch.device) + eps
            elif sde.reflow_t_schedule == 't1': # reverse distill for t=1
                t = torch.ones(batch.shape[0], device=batch.device) * sde.T
            ...
            t = torch.randn(batch.shape[0], device=batch.device) * (sde.T - eps) + eps
        ...
        if sde.reflow_flag:
            ...
            losses = torch.sqaure(score - target)
...
def get_step_fn(sde, train, optimize_fn=None, reduce_mean=False, continuous=True, likelihood_weighting=False):
    ...
    Args:
        sde: An 'sds.SDE' object that represents the forward sde
        optimize_fn: An optimization function.
        reduce_mean: If 'True', average the loss across data dimensions. Otherwise sum the loss across data dimension. Defaults to False.
    ...
    def step_fn(state, batch):
        ...
        This function will undergo 'jax.lax.scan' so that multiple steps can be pmapped and jit-compiled together for faster execution.
